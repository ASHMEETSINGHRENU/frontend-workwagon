...
function FreelancerLogo() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedGigs) {
        ...
        const size = new Blob([savedGigs]).size;
        console.log(`Loaded ${parsedGigs.length} gigs, size: ${(size / 1024).toFixed(2)} KB`);
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        try {
          // Compress the image
          const compressedImage = await compressImage(reader.result, 80); // Target 80KB
  ...
  const handleUpload = () => {
    ...
    try {
      ...
      handleClear();
      alert(`Logo design service successfully published! (Size: ${sizeInKB.toFixed(2)} KB)`);
      navigate("/dashboard");
    } catch (error) {
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          logoGigs.length > 0 && React.createElement(
            ...
            `Current gigs: ${logoGigs.length} | Storage used: ${(JSON.stringify(logoGigs).length / 1024).toFixed(2)} KB / ~5000 KB`
          )
        ),
        React.createElement(
          "div",
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                "Logo Image (Max 2MB, will be compressed)"
              ),
              React.createElement("input", {
                type: "file",
                id: "image",
