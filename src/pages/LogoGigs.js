...
import { 
  FaStar, FaPaintBrush, FaPalette, FaPenFancy, 
...
export default function LogoGigs() {
  ...
  const getDesignStyle = (title) => {
    const styles = [];
    if (title?.toLowerCase().includes('minimal')) styles.push({ icon: <FaFont />, name: 'Minimal' });
    if (title?.toLowerCase().includes('modern')) styles.push({ icon: <FaVectorSquare />, name: 'Modern' });
    if (title?.toLowerCase().includes('creative')) styles.push({ icon: <FaPalette />, name: 'Creative' });
    if (title?.toLowerCase().includes('corporate')) styles.push({ icon: <FaImage />, name: 'Corporate' });
    if (styles.length === 0) {
      styles.push({ icon: <FaPaintBrush />, name: 'Custom' });
      styles.push({ icon: <FaPenFancy />, name: 'Unique' });
    }
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <div className="relative z-10 flex-grow">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="relative bg-gradient-to-r from-purple-600 via-pink-600 to-orange-600 text-white py-16 md:py-20 overflow-hidden"
        >
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 text-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-sm mb-6">
                ...
                <span className="text-sm font-medium">Creative Designers</span>
              </div>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4">
                Professional Logo Designers
              </h1>
              <p className="text-lg md:text-xl text-purple-100 max-w-2xl mx-auto">
                Hire creative logo designers to create a unique brand identity that stands out
              </p>
            </motion.div>
          </div>
        </motion.div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="relative w-full md:w-96">
                ...
                <input
                  type="text"
                  placeholder="Search designers by name, style, or level..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-10 py-2 rounded-full border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-400 transition-all"
                />
                {searchTerm && (
                  <button
        ...
        <div className="max-w-7xl mx-auto px-4 mt-6">
          <p className="text-gray-500 text-sm">
            Showing <span className="font-semibold text-gray-700">{filteredGigs.length}</span> creative designers
          </p>
        </div>
          ...
          {filteredGigs.length === 0 ? (
            ...
            >
              {filteredGigs.map((gig) => {
                ...
                const designStyles = getDesignStyle(gig.title);
                        ...
                        <div className="relative h-48 overflow-hidden">
                          {gig.img ? (
                            ...
                            <div className="w-full h-full bg-gradient-to-br from-purple-400 to-pink-500 flex items-center justify-center">
                              <FaPaintBrush className="text-white text-6xl opacity-50" />
                            </div>
                          )}
        ...
        >
          ...
          <div className="relative z-10 p-8 md:p-12 text-center text-white">
            <h3 className="text-2xl md:text-3xl font-bold mb-3">
              Need a Unique Logo for Your Brand?
            </h3>
            <p className="text-purple-100 mb-6 max-w-2xl mx-auto">
              Get custom logo designs from top-rated creative professionals
            </p>
            <Link to="/post-project">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-6 py-3 bg-white text-purple-600 rounded-full font-semibold shadow-lg hover:shadow-xl transition-all"
              >
