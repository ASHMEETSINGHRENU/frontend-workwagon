...
export default function WebsiteGigs() {
  ...
  const getTechStack = (title) => {
    const techs = [];
    if (title?.toLowerCase().includes('react')) techs.push({ icon: <FaReact />, name: 'React' });
    if (title?.toLowerCase().includes('node')) techs.push({ icon: <FaNodeJs />, name: 'Node.js' });
    if (title?.toLowerCase().includes('wordpress')) techs.push({ icon: <FaWordpress />, name: 'WordPress' });
    if (title?.toLowerCase().includes('ecommerce')) techs.push({ icon: <FaShoppingCart />, name: 'E-commerce' });
    if (techs.length === 0) {
      techs.push({ icon: <FaCode />, name: 'Web Dev' });
      techs.push({ icon: <FaMobile />, name: 'Responsive' });
    }
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <div className="relative z-10 flex-grow">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="relative bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white py-16 md:py-20 overflow-hidden"
        >
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 text-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-sm mb-6">
                ...
                <span className="text-sm font-medium">Top Rated Freelancers</span>
              </div>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4">
                Expert Website Developers
              </h1>
              <p className="text-lg md:text-xl text-blue-100 max-w-2xl mx-auto">
                Hire the best freelance web developers to bring your digital vision to life
              </p>
            </motion.div>
          </div>
        </motion.div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="relative w-full md:w-96">
                ...
                <input
                  type="text"
                  placeholder="Search freelancers by name, skill, or level..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-10 py-2 rounded-full border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                />
                {searchTerm && (
                  <button
        ...
        <div className="max-w-7xl mx-auto px-4 mt-6">
          <p className="text-gray-500 text-sm">
            Showing <span className="font-semibold text-gray-700">{filteredGigs.length}</span> freelancers
          </p>
        </div>
          ...
          {filteredGigs.length === 0 ? (
            ...
            >
              {filteredGigs.map((gig) => {
                ...
                const techStack = getTechStack(gig.title);
